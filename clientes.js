...
const update = (req, res) => {
    ...
    let query = `UPDATE clientes SET nome = '${nome}', cpf = '${cpf}', email = '${email}', endereco = '${endereco}', data_nascimento = '${data_nascimento}', data_cadastro = '${data_cadastro}' WHERE cliente_id = ${cliente_id}`;
    con.query(query, (err, result) => {
        if (err) {
            res.status(500).json(err)
        } else {
